...
export default function AdminClient(){
  ...
  async function load(){
    ...
    if(m.status===401||o.status===401){setAuth("out");return;}
    if(!m.ok||!o.ok){setError("Could not load dashboard data.");return;}
  ...
  if(auth==="loading") return <div className="login"><div className="loginCard"><div className="brand"><span className="brandMark">P</span><span style={{color:'#111'}}>PETROCRIB</span></div><h1>Loading dashboard…</h1></div></div>;
  if(auth==="out") return <div className="login"><form className="loginCard" onSubmit={login}><div className="brand"><span className="brandMark">P</span><span style={{color:'#111'}}>PETROCRIB</span></div><h1>Admin login</h1><p>Sales, visitors, carts and fulfilment in one place.</p><input type="password" placeholder="Admin password" value={password} onChange={e=>setPassword(e.target.value)} autoFocus/><button className="btn yellow">Open dashboard</button>{error&&<div className="error">{error}</div>}</form></div>;

  const s=metrics?.summary||{};
  return <div className="adminShell">
    <header className="topbar"><div className="brand"><span className="brandMark">P</span><span>PETROCRIB ADMIN</span></div><div className="topMeta"><span className="refreshDot"/>Auto-refreshing · {last?last.toLocaleTimeString():"—"}</div></header>
    <main className="main">
      <div className="heroRow"><div><div className="eyebrow">Store pulse</div><h1>Business dashboard</h1><p>Traffic → cart → payment → fulfilment.</p></div><button className="btn" onClick={load}>Refresh now</button></div>
