/* PETROCRIB FX — site-wide interactivity
   - 3D tilt on product cards & hero collage (follows cursor)
   - magnetic buttons
   - scroll-reveal for cards
   - yellow cursor glow spotlight
   All effects disabled on touch devices / reduced-motion. */
...
(function () {
  ...
  /* ---- Chitram hero: layered depth parallax across full width ---- */
  const hero = document.getElementById("chitramHero");
  if (hero) {
    const art = hero.querySelector(".hero-art img");
    const title = hero.querySelector(".hero-title-img");
    const bees = hero.querySelectorAll(".cs-bee");
    ...
    hero.addEventListener("mousemove", (e) => {
      ...
      if (art) art.style.transform =
        `translate(${px * -14}px, ${py * -10}px) scale(1.06)`;
      if (title) title.style.transform = `translate(${px * 16}px, ${py * 10}px)`;
      bees.forEach((b, i) => {
        const d = depth[i] || 1;
  ...
  /* ---- 3D tilt (delegated, works on dynamic cards) ---- */
  const TILT_SEL = ".card, .hero-collage a, .gallery .main";
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    b.style.transform = `translate(${dx * 0.15}px, ${dy * 0.25}px)`;
  });
  document.addEventListener(
    "mouseout",
    (e) => {
  ...
  // cards render async — watch for them
