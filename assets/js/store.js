  ...
  grid.innerHTML = products.map((p) => {
    ...
    return `
      <a class="card" href="product.html?handle=${encodeURIComponent(p.handle)}">
        ...
        <div class="meta"><h3>${escapeHtml(p.title)}</h3><div class="price">${priceLabel}</div></div>
      </a>`;
  }).join("");
}
...
const SIZE_GUIDES = {
  regular: {
    label: "Regular fit — size guide (inches)", cols: ["Size", "Chest", "Length"],
    rows: [["S",36,26],["M",38,27],["L",40,28],["XL",42,29],["2XL",44,30],["3XL",48,31],["4XL",50,32],["5XL",52,33]],
  },
  oversized: {
    label: "Oversized fit — size guide (inches)", cols: ["Size", "Chest", "Length", "Shoulder", "Sleeve"],
    rows: [["S",44,28,21,8],["M",46,29,22,8.5],["L",48,30,23,9],["XL",50,30,24,9.5],["XXL",52,30.5,25,10]],
  },
};
function guideForType(typeName) { return (typeName || "").toLowerCase().includes("regular") ? SIZE_GUIDES.regular : SIZE_GUIDES.oversized; }
function guideTable(g) {
  return `<div class="size-guide"><div class="sg-label">${g.label}</div><table><thead><tr>${g.cols.map((c)=>`<th>${c}</th>`).join("")}</tr></thead><tbody>${g.rows.map((r)=>`<tr>${r.map((v)=>`<td>${v}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`;
}
...
async function renderProduct() {
  ...
  function currentPrice() { return Math.round(p.typePrices[state.type]); }
  function summary() { return `${p.title} | ${state.type} | ${cap(state.color)} | Size ${state.size}`; }
  ...
  function paint() {
    ...
    if (link) noteEl.innerHTML = `On the payment page, mention: <b>${escapeHtml(summary())}</b> (we copy it to your clipboard when you tap Buy).`;
    else if (STORE_CONFIG.whatsappNumber) noteEl.textContent = "Ordering via WhatsApp — your selection is pre-filled in the message.";
    else noteEl.textContent = "Ordering is not set up yet. Store owner: add your Razorpay links in config.js.";
  }
  function setActive(id,val){const box=document.getElementById(id);if(!box)return;box.querySelectorAll(".pill").forEach((b)=>b.classList.toggle("on",b.dataset.v===val));}
  ...
  buyBtn.addEventListener("click",async()=>{
    ...
    const price=currentPrice(),link=STORE_CONFIG.razorpayLinks[price];
    if(link){try{await navigator.clipboard.writeText(summary())}catch(e){} window.open(link,"_blank");}
    else if(STORE_CONFIG.whatsappNumber){const msg=encodeURIComponent(`Hi! I want to order:\n${summary()}\nPrice: ${money(price)}`);window.open(`https://wa.me/${STORE_CONFIG.whatsappNumber}?text=${msg}`,"_blank");}
    else alert("Ordering is not configured yet. Add Razorpay links in config.js.");
  });
  function bindPills(id,fn){const box=document.getElementById(id);if(!box)return;box.addEventListener("click",(e)=>{const b=e.target.closest(".pill");if(b)fn(b.dataset.v);});}
  paint();
}
...
(function fillFooter(){
  const ig=document.getElementById("igLink");if(ig){if(STORE_CONFIG.instagram)ig.href="https://instagram.com/"+STORE_CONFIG.instagram.replace("@","");else ig.style.display="none";}
  const wa=document.getElementById("waLink");if(wa){if(STORE_CONFIG.whatsappNumber)wa.href="https://wa.me/"+STORE_CONFIG.whatsappNumber;else wa.style.display="none";}
  const set=(id,val,prefix)=>{const el=document.getElementById(id);if(el)el.textContent=val?(prefix||"")+val:"";};
  set("fEmail",STORE_CONFIG.email);set("fPhone",STORE_CONFIG.phone);set("fAddress",STORE_CONFIG.address);
})();
